...
def get_multitree(voc_fd, lang_id):
	...
	for letter in distractors:
		distractors_tree[letter] = pybktree.BKTree(ed.edit_distance, distractors[letter])
...
if __name__ == "__main__":
	(n_lines, n_types, n_tokens, n_missing) = generate_distractors(sys.argv[1], sys.argv[2])

	print('%d processed. %d types. %d tokens. %d tokens miss distractors.' % (n_lines, n_types, n_tokens, n_missing))
